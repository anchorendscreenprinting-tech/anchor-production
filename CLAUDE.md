...
- Context if a write path is ever reconsidered (don't): `anchor-hq/firestore.rules` guards the doc with BOTH `appVersion` (numeric, non-decreasing — blocks stale code) and, **live since 2 Jul 2026**, a monotonic `revision` (every write must set the stored value + 1 — blocks stale data, closing the same-version last-writer-wins gap). Any writer that isn't ink-library's revision-aware client is simply rejected. The clean answer stands: this app doesn't write the doc.
- **`hq_recipe_requests` (own collection — NOT part of inklib):** one-way "print a recipe label for this pot" notifications. **anchor-production writes** (request created when a colour is flagged "Needs recipe" in the Inks Mixed checklist; closed when the flag is untapped or the colour is ticked as mixed) and **anchor-hq writes** (closed via "Done" in its Recipes inbox); **anchor-hq reads** live (onSnapshot + nav badge, enquiries-inbox pattern). Flag state also persists per colour as `recipeRequested` inside the job's `inkPrep`. Nothing travels back to production. Covered by the open write rule (`coll != 'inklib'`).
...
- **Firestore rule matches are OR'd — a broad catch-all silently deadens stricter sub-rules.** Any single matching `allow` grants the request, so `match /{document=**} { allow write: if true }` makes every other write rule in the file decorative. The old console-only ruleset had exactly this bug; the current `anchor-hq/firestore.rules` is structured to avoid it (write rules exclude the guarded path instead of relying on a stricter match to win — it can't).
- **Firestore does NOT extend a parent match to subcollections.** `match /coll/{doc}` covers only that document, not anything nested under it — a rules draft this sprint would have silently denied all subcollection writes because of this. Use recursive `{rest=**}` (matches zero or more segments in rules v2) when a rule must cover nested paths.
- **Never normalise or rewrite stored data on READ.** Reads stay pure; data changes happen only via explicit user action. In ink-library, a leftover on-read code normaliser re-saved every snapshot and silently reverted user corrections for days. If a data shape looks wrong at read time, surface it — don't "fix" it in the read path.
- **Pantone colour data is GENERATED — never hand-edit.** `anchor-hq/scripts/build-pantone.mjs` generates both `anchor-hq/js/pantone-data.js` and `ink-library/js/pantone-colors.js` (3,233 colours) in one run. To change the data, edit and re-run the generator — never the generated files.
